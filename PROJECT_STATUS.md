...
### 21. Test-session auto-tagging infrastructure — new this session, backend done, frontend Test Mode toggle not yet built
User's core, ongoing pain: repeated feature testing on staging kept polluting real stock/dashboard calculations, requiring manual `isTestData` cleanup after every test (this whole session's cleanup steps are a case study in why). Solution landed in two pieces so far (see "Not yet started" for what's left):
...
Still needed: customers, vendors, additional warehouses/racks/bins beyond the one `TEST-WH` rack set up this session, and an actual end-to-end live test (Sales Order → routing-chain Work Orders) to prove the rebuilt chains work together, not just that each piece exists.
...
- **Test-session frontend Test Mode toggle** — item 21's backend auto-tagging is done, but there's no way to trigger it from the actual UI yet (only via the `X-Test-Session` curl header). Needs a toggle (nav bar or Settings) that attaches the header to every request while enabled, with a persistent visual banner so it can't be left on by accident.
- **Test-session dashboard filtering** — even with auto-tagging working, the key reporting endpoints (stock balance, production dashboard, MRP shortage checks) don't yet exclude `isTestData: true` records from their calculations. Auto-tagging alone doesn't hide test data from real numbers - this filtering step is what actually would.
...
- **Master data rebuild** — customers, vendors, additional warehouses/racks/bins beyond the one `TEST-WH` rack, and a live end-to-end test of the 3 rebuilt routing chains (Sales Order → Work Orders) — see item 24.
