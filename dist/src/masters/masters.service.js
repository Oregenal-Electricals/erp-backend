...
function isSuperUser(user) {
    return (user === null || user === void 0 ? void 0 : user.role) === client_1.UserRole.SUPER_ADMIN || (user === null || user === void 0 ? void 0 : user.role) === client_1.UserRole.CORPORATE_ADMIN;
}
let MastersService = class MastersService {
    ...
    async findAllCompanies(includeInactive = false, user) {
    ...
    async updateCompany(id, dto, userId) {
        ...
        const updated = await this.prisma.company.update({ where: { id }, data: Object.assign(Object.assign({}, dto), { updatedBy: userId }) });
    ...
    async createPlant(dto, userId) {
        ...
        const plant = await this.prisma.plant.create({ data: Object.assign(Object.assign({}, dto), { createdBy: userId, updatedBy: userId }) });
        await this.audit.log({ tableName: 'plants', recordId: plant.id, action: 'CREATE', newValues: plant, changedBy: userId });
        return plant;
    }
    async findAllPlants(companyId, includeInactive = false, user) {
        const where = Object.assign({}, (includeInactive ? {} : { isActive: true }));
        if (companyId) {
    ...
    async updatePlant(id, dto, userId) {
        ...
        const updated = await this.prisma.plant.update({ where: { id }, data: Object.assign(Object.assign({}, dto), { updatedBy: userId }) });
    ...
    async createUnit(dto, userId) {
        ...
        const unit = await this.prisma.unit.create({ data: Object.assign(Object.assign({}, dto), { createdBy: userId, updatedBy: userId }) });
        await this.audit.log({ tableName: 'units', recordId: unit.id, action: 'CREATE', newValues: unit, changedBy: userId });
        return unit;
    }
    async findAllUnits(plantId, includeInactive = false, user) {
        const where = Object.assign({}, (includeInactive ? {} : { isActive: true }));
        if (plantId) {
    ...
    async updateUnit(id, dto, userId) {
        ...
        const updated = await this.prisma.unit.update({ where: { id }, data: Object.assign(Object.assign({}, dto), { updatedBy: userId }) });
    ...
    async createDepartment(dto, userId) {
        ...
        const dept = await this.prisma.department.create({ data: Object.assign(Object.assign({}, dto), { createdBy: userId, updatedBy: userId }) });
        await this.audit.log({ tableName: 'departments', recordId: dept.id, action: 'CREATE', newValues: dept, changedBy: userId });
        return dept;
    }
    async findAllDepartments(companyId, includeInactive = false, user) {
        const where = Object.assign({}, (includeInactive ? {} : { isActive: true }));
        if (companyId) {
            where.companyId = companyId;
        }
    ...
    async updateDepartment(id, dto, userId) {
        ...
        const updated = await this.prisma.department.update({ where: { id }, data: Object.assign(Object.assign({}, dto), { updatedBy: userId }) });
    ...
    async createBranch(dto, userId) {
        ...
        const branch = await this.prisma.branch.create({ data: Object.assign(Object.assign({}, dto), { createdBy: userId, updatedBy: userId }) });
        await this.audit.log({ tableName: 'branches', recordId: branch.id, action: 'CREATE', newValues: branch, changedBy: userId });
        return branch;
    }
    async findAllBranches(companyId, includeInactive = false, user) {
        const where = Object.assign({}, (includeInactive ? {} : { isActive: true }));
        if (companyId) {
            where.companyId = companyId;
        }
    ...
    async updateBranch(id, dto, userId) {
        ...
        const updated = await this.prisma.branch.update({ where: { id }, data: Object.assign(Object.assign({}, dto), { updatedBy: userId }) });
    ...
    async findAllFinancialYears(companyId, user) {
        const where = {};
        if (companyId) {
            where.companyId = companyId;
        }
