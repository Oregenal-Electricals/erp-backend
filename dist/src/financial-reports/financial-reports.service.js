...
let FinancialReportsService = class FinancialReportsService {
    ...
    getPeriodDates(period, fromDate, toDate) {
        if (fromDate && toDate)
    ...
    async getTrialBalance(user, query) {
        ...
        const accountTotals = {};
        entries.forEach(e => {
            if (!accountTotals[e.accountId])
                accountTotals[e.accountId] = { debit: 0, credit: 0 };
            if (e.entryType === 'DEBIT')
                accountTotals[e.accountId].debit += e.amount;
            else
                accountTotals[e.accountId].credit += e.amount;
        });
        const rows = accounts.map(acc => {
            ...
            return ({
                ...
                periodDebit: ((_a = accountTotals[acc.id]) === null || _a === void 0 ? void 0 : _a.debit) || 0,
                periodCredit: ((_b = accountTotals[acc.id]) === null || _b === void 0 ? void 0 : _b.credit) || 0,
    ...
    async getProfitAndLoss(user, query) {
        const { fromDate, toDate } = this.getPeriodDates(query.period, query.fromDate, query.toDate);
        const accounts = await this.prisma.account.findMany({
    ...
    async getBalanceSheet(user, query) {
