...
let SalarySlipService = class SalarySlipService {
    constructor(prisma) {
        this.prisma = prisma;
    }
    async generateSlip(employeeId, month, year, companyId) {
        ...
        if (!entry)
            throw new common_1.NotFoundException(`No payroll entry found for employee ${employeeId} for ${month}/${year}`);
        const company = await this.prisma.company.findUnique({ where: { id: companyId } });
        return this.buildPdf(entry, company);
    }
    async generateBulkSlips(payrollRunId, companyId) {
        ...
        if (!run.entries.length)
            throw new common_1.NotFoundException('No entries in this payroll run');
        const company = await this.prisma.company.findUnique({ where: { id: companyId } });
        ...
        for (const entry of run.entries) {
            doc.addPage();
            await this.renderSlipPage(doc, Object.assign(Object.assign({}, entry), { payrollRun: { runNumber: run.runNumber, status: run.status } }), company);
        }
        doc.end();
        return new Promise(resolve => doc.on('end', () => resolve(Buffer.concat(chunks))));
    }
    async buildPdf(entry, company) {
    ...
    async renderSlipPage(doc, entry, company) {
        var _a, _b, _c, _d, _e;
        ...
        doc.fontSize(16).font('Helvetica-Bold').fillColor('#ffffff')
            .text((company === null || company === void 0 ? void 0 : company.name) || 'Acme Electronics Pvt Ltd', 55, 52);
        doc.fontSize(9).font('Helvetica').fillColor('#bfdbfe')
            .text(`GSTIN: ${(company === null || company === void 0 ? void 0 : company.gstin) || 'XXXXXXXXXXXX'} | ${(company === null || company === void 0 ? void 0 : company.address) || ''}`, 55, 72);
        ...
        const empFields = [
            ...
            ['Designation', ((_b = emp.designation) === null || _b === void 0 ? void 0 : _b.name) || '—'],
        ];
        const empFields2 = [
        ...
        const attFields = [
            ...
            ['OT Hours', ((_c = entry.otHours) === null || _c === void 0 ? void 0 : _c.toFixed(1)) + 'h'],
        ];
        ...
        for (let i = 0; i < maxRows; i++) {
            ...
            if (earnings[i]) {
                ...
                doc.font('Helvetica-Bold').fillColor('#111827')
                    .text(fmtAmt(earnings[i][1]), 200, rowY + 5, { width: 90, align: 'right' });
            }
            if (deductions[i]) {
                doc.fontSize(8).font('Helvetica').fillColor('#374151')
                    .text(deductions[i][0], colMid + 10, rowY + 5, { width: 150 });
                doc.font('Helvetica-Bold').fillColor('#dc2626')
                    .text(fmtAmt(deductions[i][1]), colMid + 160, rowY + 5, { width: 90, align: 'right' });
            }
        }
        y += maxRows * rowH;
        ...
        doc.fontSize(7).font('Helvetica').fillColor('#9ca3af')
            .text(`Payroll Run: ${((_d = entry.payrollRun) === null || _d === void 0 ? void 0 : _d.runNumber) || '—'} | Status: ${((_e = entry.payrollRun) === null || _e === void 0 ? void 0 : _e.status) || '—'} | Generated: ${new Date().toLocaleDateString('en-IN')}`, 40, y, { align: 'center', width: 515 });
    ...
    async getSlipHistory(companyId, employeeId) {
...
exports.SalarySlipService = SalarySlipService = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [prisma_service_1.PrismaService])
], SalarySlipService);
//# sourceMappingURL=salary-slip.service.js.map