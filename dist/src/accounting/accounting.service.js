...
let AccountingService = class AccountingService {
    ...
    async seedDefaultCoa(companyId, userId) {
        const existing = await this.prisma.accountGroup.count({ where: { companyId } });
        if (existing > 0)
            return { message: 'COA already seeded', groups: existing };
        const groupMap = {};
        for (const g of DEFAULT_GROUPS) {
            const group = await this.prisma.accountGroup.create({
                data: Object.assign(Object.assign({}, g), { companyId, isSystemGroup: true, createdBy: userId, updatedBy: userId }),
            });
            groupMap[g.code] = group.id;
        }
        let accountCount = 0;
        for (const a of DEFAULT_ACCOUNTS) {
            const { groupCode } = a, accountData = __rest(a, ["groupCode"]);
            const groupId = groupMap[groupCode];
            if (!groupId)
                continue;
            await this.prisma.accountHead.create({
                data: Object.assign(Object.assign({}, accountData), { groupId, companyId, isSystemAccount: true, createdBy: userId, updatedBy: userId }),
            });
            accountCount++;
        }
    ...
    async createGroup(dto, user) {
        ...
        const group = await this.prisma.accountGroup.create({ data: Object.assign(Object.assign({}, dto), { companyId: user.companyId, createdBy: user.id, updatedBy: user.id }) });
    ...
    async updateGroup(id, dto, user) {
        ...
        if (group.isSystemGroup)
            throw new common_1.BadRequestException('Cannot edit system group');
        return this.prisma.accountGroup.update({ where: { id }, data: Object.assign(Object.assign({}, dto), { updatedBy: user.id }) });
    }
    async findAllGroups(user) {
    ...
    async createAccount(dto, user) {
        ...
        const account = await this.prisma.accountHead.create({
            data: Object.assign(Object.assign({}, dto), { companyId: user.companyId, type: group.type, nature: group.nature, currentBalance: dto.openingBalance || 0, createdBy: user.id, updatedBy: user.id }),
    ...
    async updateAccount(id, dto, user) {
        ...
        if (account.isSystemAccount && dto.code)
            throw new common_1.BadRequestException('Cannot change code of system account');
        const updated = await this.prisma.accountHead.update({ where: { id }, data: Object.assign(Object.assign({}, dto), { updatedBy: user.id }), include: { group: { select: { name: true } } } });
        await this.audit.log({ tableName: 'account_heads', recordId: id, action: 'UPDATE', newValues: updated, changedBy: user.id });
        return updated;
    }
    async findAllAccounts(user, query) {
