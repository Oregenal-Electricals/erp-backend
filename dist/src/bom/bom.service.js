...
let BomService = class BomService {
    constructor(prisma, audit, notifications, workflows) {
        this.prisma = prisma;
        this.audit = audit;
    ...
    async findAll(user, query) {
        ...
        const rank = (s) => (s === 'APPROVED' ? 3 : s === 'PENDING_APPROVAL' ? 2 : s === 'DRAFT' ? 1 : 0);
    ...
    async submitForApproval(id, user) {
        const bom = await this.findOne(id, user);
        if (bom.status !== 'DRAFT')
            throw new common_1.BadRequestException('Only DRAFT BOMs can be submitted for approval');
        if (!bom.items || bom.items.length === 0)
            throw new common_1.BadRequestException('Cannot submit BOM with no items for approval');
        const openQuery = await this.prisma.bomQuery.findFirst({ where: { bomId: id, status: 'OPEN' } });
        if (openQuery)
            throw new common_1.BadRequestException('Cannot submit this BOM for approval while a query on it is still open');
    ...
    async raiseQuery(dto, user) {
        ...
        const chainApproverIds = (((_a = request === null || request === void 0 ? void 0 : request.workflow) === null || _a === void 0 ? void 0 : _a.steps) || []).filter((s) => { var _a; return s.level <= ((_a = request === null || request === void 0 ? void 0 : request.currentLevel) !== null && _a !== void 0 ? _a : 0); }).map((s) => s.approverUserId).filter(Boolean);
            ...
            if (existingRm) {
                ...
                const newRm = await client.rawMaterial.create({
                    data: {
                        ...
                        partNumber: item.itemCode, uomId: uomRecord === null || uomRecord === void 0 ? void 0 : uomRecord.id,
                        createdBy: user.id, updatedBy: user.id,
                    },
                });
    ...
    async getStats(user) {
        const where = { isActive: true, bomType: 'MASTER' };
        if (user.role !== 'SUPER_ADMIN')
            where.companyId = user.companyId;
        const now = new Date();
        const activeWhere = Object.assign(Object.assign({}, where), { status: 'APPROVED', OR: [{ effectiveTo: null }, { effectiveTo: { gt: now } }] });
