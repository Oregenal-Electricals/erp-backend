...
let OqcService = class OqcService {
    ...
    async create(dto, user) {
        if (!dto.fgReceiptId)
            throw new common_1.BadRequestException('fgReceiptId is required');
        ...
        if (fgReceipt.status !== 'RECEIVED')
            throw new common_1.BadRequestException('FG Receipt must be RECEIVED before OQC can be raised against it');
        const existing = await this.prisma.oqcInspection.findFirst({
            where: { fgReceiptId: dto.fgReceiptId, companyId: user.companyId },
        });
        if (existing)
            throw new common_1.BadRequestException(`OQC ${existing.oqcNumber} already exists for this FG Receipt`);
    ...
    async getPendingFgReceipts(user) {
