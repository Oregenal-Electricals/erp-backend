...
let ApService = class ApService {
    ...
    async generateBillNumber(companyId) {
    ...
    async getBillableSummary(poId, companyId) {
        ...
        const totalAcceptedValue = grns.reduce((sum, grn) => {
            return sum + grn.items.reduce((s, item) => {
    ...
    async create(dto, user) {
        ...
        if (dto.poId) {
            ...
            if (dto.totalAmount > summary.maxAllowed) {
                throw new common_1.BadRequestException(`This invoice (${dto.totalAmount}) exceeds what's billable against this PO. ` +
    ...
    async createBillVoucher(bill, user) {
        ...
        if (cgstInput && bill.totalGst > 0) {
            ...
            if (sgstInput)
                entries.push({ accountId: sgstInput.id, entryType: 'DEBIT', amount: bill.totalGst - cgstAmt, narration: `SGST Input - ${bill.billNumber}`, companyId: user.companyId, createdBy: user.id, updatedBy: user.id });
        }
        ...
        if (cgstInput && bill.totalGst > 0) {
            ...
            if (sgstInput)
                await this.prisma.account.update({ where: { id: sgstInput.id }, data: { currentBalance: { increment: bill.totalGst - cgstAmt } } });
        }
    ...
    async recordPayment(dto, user) {
        ...
        if (dto.amount > bill.outstandingAmount)
            throw new common_1.BadRequestException(`Payment ${dto.amount} exceeds outstanding ${bill.outstandingAmount}`);
    ...
    async createPaymentVoucher(payment, bill, dto, user) {
        const creditors = await this.prisma.account.findFirst({ where: { companyId: user.companyId, accountCode: '2001' } });
    ...
    async getStats(user) {
        ...
        const [total, approved, partial, paid, overdue] = await Promise.all([
            this.prisma.apBill.count({ where }),
            this.prisma.apBill.count({ where: Object.assign(Object.assign({}, where), { status: 'APPROVED' }) }),
            this.prisma.apBill.count({ where: Object.assign(Object.assign({}, where), { status: 'PARTIAL' }) }),
            this.prisma.apBill.count({ where: Object.assign(Object.assign({}, where), { status: 'PAID' }) }),
            this.prisma.apBill.count({ where: Object.assign(Object.assign({}, where), { status: { in: ['APPROVED', 'PARTIAL'] }, dueDate: { lt: now } }) }),
        ]);
        const outstanding = await this.prisma.apBill.aggregate({ where: Object.assign(Object.assign({}, where), { status: { in: ['APPROVED', 'PARTIAL', 'OVERDUE'] } }), _sum: { outstandingAmount: true } });
    ...
    async getAgingReport(user) {
        ...
        const agingBills = bills.map(b => {
            ...
            if (days > 90)
                aging.over90 += b.outstandingAmount;
            else if (days > 60)
                aging.days90 += b.outstandingAmount;
            else if (days > 30)
                aging.days60 += b.outstandingAmount;
            else if (days > 0)
                aging.days30 += b.outstandingAmount;
            else
                aging.current += b.outstandingAmount;
            return Object.assign(Object.assign({}, b), { agingDays: days });
        });
