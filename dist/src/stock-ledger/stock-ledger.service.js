...
let StockLedgerService = class StockLedgerService {
    ...
    async receiveFromOqc(oqcId, user) {
        const oqc = await this.prisma.oqcInspection.findFirst({
            where: { id: oqcId, companyId: user.companyId },
            include: { fgReceipt: true },
        });
        if (!oqc)
            throw new common_1.NotFoundException('OQC inspection not found');
        if (oqc.status !== 'RELEASED')
            throw new common_1.BadRequestException('OQC must be RELEASED');
        if (!oqc.fgReceipt)
            throw new common_1.BadRequestException('OQC has no linked FG Receipt to credit stock for');
        const alreadyReceived = await this.prisma.stockLedger.findFirst({
            where: { companyId: user.companyId, referenceType: 'OQC', referenceId: oqcId },
        });
        if (alreadyReceived)
            throw new common_1.BadRequestException(`Stock has already been received for ${oqc.oqcNumber}`);
