...
let StageTransferService = class StageTransferService {
    ...
    async giveToQc(dto, user) {
        var _a;
        const fromWo = await this.prisma.workOrder.findFirst({ where: { id: dto.fromWorkOrderId, companyId: user.companyId } });
        if (!fromWo)
            throw new common_1.NotFoundException('Source Work Order not found');
        if (!['IN_PROGRESS', 'COMPLETED'].includes(fromWo.status)) {
            throw new common_1.BadRequestException('Only a Work Order that is IN PRODUCTION or COMPLETED has output to hand to QC');
        }
        if (fromWo.routingGroupId) {
            ...
            const thisStage = routingStages.find((s) => s.stageName === fromWo.stageName);
            if (thisStage && routingStages.length > 0) {
                const maxSequence = Math.max(...routingStages.map((s) => s.sequence));
                if (thisStage.sequence !== maxSequence) {
                    throw new common_1.BadRequestException(`${fromWo.stageName} is not the final production stage in this routing - QC handover is only valid from the last operation`);
                }
            }
        }
        const transferable = fromWo.completedQty - fromWo.cumulativeHandoverQty;
        ...
        if (qty > transferable) {
            throw new common_1.BadRequestException(`Cannot give ${qty} to QC - only ${transferable} is transferable (${fromWo.completedQty} completed minus ${fromWo.cumulativeHandoverQty} already given)`);
        }
        const updated = await this.prisma.$executeRaw `
      UPDATE work_orders SET "cumulativeHandoverQty" = "cumulativeHandoverQty" + ${qty}, "updatedBy" = ${user.id}
      WHERE id = ${fromWo.id} AND "completedQty" - "cumulativeHandoverQty" >= ${qty}
    `;
        ...
        await this.audit.log({ tableName: 'stage_transfer_notes', recordId: note.id, action: 'CREATE', newValues: Object.assign(Object.assign({}, note), { isQcHandover: true }), changedBy: user.id });
        return note;
    }
